#ifndef ATTENUATIONZ_HH
#define ATTENUATIONZ_HH
...
public:
    ...
    void SetWaterParameters(double abw, double ray)
    {
        ABWFF = abw;
        RAYFF = ray;
        SetAttenuationLength();
    ...
    void SetAttenuationLength()
    {
        ...
        TGraph gr_abs(NUMENTRIES_water,ENERGY_water,ABSORPTION_water);
        TGraph gr_ray(NUMENTRIES_water,ENERGY_water,RAYLEIGH_water);
        double photoEnergy = 1239.84193/wavelength;
        alpha0 = 1./(1./gr_abs.Eval(photoEnergy)+1./gr_ray.Eval(photoEnergy));
        ...
        std::cout<<"Attenutation length = "<<alpha0<<" cm"<<std::endl;
    }

    double GetAttenuationZWeight(double R, double dz)
    {
        double alpha_z0 = alpha0 + slopeA*source_zpos;
        //std::cout<<"alpha_z0 = "<<alpha_z0<<std::endl;
        double old_weight = exp(-R/alpha0);
        double new_weight = TMath::Power(1.+slopeA*dz/alpha_z0, -R/slopeA/dz);
